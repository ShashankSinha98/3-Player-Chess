...
public class JChessTabbedPane extends JTabbedPane implements MouseListener, ImageObserver
{
    ...
    {
        if (JChessApp.getJChessView().newGameFrame == null)
        {
            JChessApp.getJChessView().newGameFrame = new NewGameWindow();
        }
        JChessApp.getApplication().show(JChessApp.getJChessView().newGameFrame);
    }
